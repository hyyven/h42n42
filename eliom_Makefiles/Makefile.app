...
config-files: | $(TEST_PREFIX)$(ELIOMSTATICDIR) $(TEST_PREFIX)$(LIBDIR)
	...
	cp -f _build/default/src/$(PROJECT_NAME).cm* $(TEST_PREFIX)$(LIBDIR)/
...
byte:: js
	$(ENV_PSQL) dune build $(DUNE_OPTIONS) src/$(PROJECT_NAME)_main.bc
	make config-files PROJECT_NAME=$(PROJECT_NAME)

opt:: js
	$(ENV_PSQL) dune build $(DUNE_OPTIONS) src/$(PROJECT_NAME)_main.exe
	make config-files PROJECT_NAME=$(PROJECT_NAME)
